//1. Disconnect the both paired ble by at command!
//2. Retrive string from remote bluetooth.
//3. Install Two BLE and test code by SoftwareSerial.
#include <SoftwareSerial.h>  // 使用arduino內建(軟體序列埠)程式庫
SoftwareSerial BTSerial(10, 11);    // 設定軟體序列埠(接收腳RX,傳送腳TX)
char input;                  // 接收序列埠值的變數input資料型態為字元

int16_t abc=16666;
int qq = 0;
int initDelay = 0;
boolean startScan = false;
boolean connectedandTesting = false;  //true;
int connectedandTestingQ = 0;
boolean queryBaud = true;  //AT+BAUD=?

String SerialCommand = "";
String ATReturn = "";

void setup()
{
  pinMode(13, OUTPUT);
  digitalWrite(13, HIGH);
  BTSerial.begin(115200);             // 啟動軟體序列埠,設定傳輸速度
  Serial.begin(115200);
}

void loop()
{
  
  if (queryBaud){
    Serial.print("Hello\r\n");
    queryBaud = false;
  }
  
  if (BTSerial.available() > 0)  //當arduino+藍牙收到手機 資料
  {
    delay(5);
    while ( BTSerial.available() ){
      delay(1); // Because return value is too long
      char c = BTSerial.read();
      ATReturn += c;
    }
      //Serial.write( BTSerial.read() );     
//      char BTSerialValue[50] = {'\0'};
//      ATReturn.toCharArray(BTSerialValue, 50);
//      Serial.write( BTSerialValue );
      Serial.print( ATReturn );
      
    
//      if( ATReturn.substring(0,4) == "Show"){
//        digitalWrite(13, LOW);
//        delay(100); 
//        digitalWrite(13, HIGH);
//      }    
      
      ATReturn = "";
  }
  
  if (Serial.available() > 0)  //check if there's any data sent from the local serial terminal, you can add the other applications here
  { 
    delay(5);
    while ( Serial.available() ){
      delay(3);
      char c = Serial.read();
      SerialCommand += c;
    }
    
    if( SerialCommand.length()==1 ){
      switch (SerialCommand[0])
      {
      case  'a' :     // 接收到 'a'，做您要的動作1
        Serial.println("AT\r");
        BTSerial.write("AT\r\n");
        break;
      case  'b' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+NAMEMT-BIKE-PERIPHERAL\r");
        BTSerial.write("AT+NAMEMT-BIKE-PERIPHERAL\r\n");
        break;
      case  'c' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+PIN835865\r");
        BTSerial.write("AT+PIN835865\r\n");
        break;
      case  'd' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+BAUD8\r");
        BTSerial.write("AT+BAUD8\r\n");
        break;
      case  'e' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+ROLE1\r");
        BTSerial.write("AT+ROLE1\r\n");
        break;
      case  'f' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+RNAME80,C1,BA,73,11,A9\r");
        BTSerial.write("AT+RNAME80,C1,BA,73,11,A9\r\n");
        break;
      case  'g' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+STATE\r");
        BTSerial.write("AT+STATE\r\n");
        break;
      case  'h' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+LADDR\r");
        BTSerial.write("AT+LADDR\r\n");
        break;
      case  'i' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+LSP\r");
        BTSerial.write("AT+LSP\r\n");
        break;
      case  'j' :     // 接收到 'b'，做您要的動作2        
        Serial.println("AT+INQM0,8,5\r");
        BTSerial.write("AT+INQM0,8,5\r\n");
        break;
      case  'k' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+AUTOINQ0\r");
        BTSerial.write("AT+AUTOINQ0\r\n");
        break;
      case  'l' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+AUTOCONN0\r");
        BTSerial.write("AT+AUTOCONN0\r\n");
        break;
      case  'm' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+AUTOINQ1\r");
        BTSerial.write("AT+AUTOINQ1\r\n");
        break;
      case  'n' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+AUTOCONN1\r");
        BTSerial.write("AT+AUTOCONN1\r\n");
        break;
      case  'o' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+CONNECT80,C1,BA,70,E3,B5\r");
        BTSerial.write("AT+CONNECT80,C1,BA,70,E3,B5\r\n");
        break;
      case  'p' :     // 接收到 'b'，做您要的動作2
         Serial.println("AT+BIND80,C1,BA,70,E3,B5\r");
        BTSerial.write("AT+BIND80,C1,BA,70,E3,B5\r\n");
        break;
      case  'q' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+BIND80,C1,BE,25,8C,43\r");
        BTSerial.write("AT+BIND80,C1,BE,25,8C,43\r\n");
        break;    
      case  'r' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+CMODE1\r");
        BTSerial.write("AT+CMODE1\r\n");
        break; 
      case  's' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+BIND\r");
        BTSerial.write("AT+BIND\r\n");
        break;  
      case  't' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+INQ\r");
        BTSerial.write("AT+INQ\r\n");
        break;
      case  'u' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+CLEAR\r");
        BTSerial.write("AT+CLEAR\r\n");
        break;    
      case  'v' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+RESET\r");
        BTSerial.write("AT+RESET\r\n");
        break;
      case  'w' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+RESETPDL\r");
        BTSerial.write("AT+RESETPDL\r\n");
        break;  
      case  'z' :     // 接收到 'b'，做您要的動作2
        Serial.println("AT+WIOMODE\r");
        BTSerial.write("AT+WIOMODE\r\n");
        break;        
      case  'x' :     // 接收到 'b'，做您要的動作2
        digitalWrite(13, HIGH);
        break; 
      case  'y' :     // 接收到 'b'，做您要的動作2
        digitalWrite(13, LOW);
        break;  

      }
      
      SerialCommand = "";
    }
    else{
      Serial.println(SerialCommand);
      //BTSerial.write(SerialCommand);
      char BTSerialValue[50] = {'\0'};
      SerialCommand.toCharArray(BTSerialValue, 50);
      BTSerial.write( BTSerialValue );
      
      SerialCommand = "";
    }
    
  }
}
